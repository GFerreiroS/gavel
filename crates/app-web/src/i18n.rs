...
pub const EXTERNAL_STRINGS: &[&str] = &[
    ...
    // §9/§10 cross-realm views. Panel values are made in Rust and reach the
    // shared panel macro dynamically, so the template scan cannot find them.
    "Realm arbitrage",
    "{} of {} realms listing",
    "latest snapshot on each collected realm",
    "gold per item",
    "Where is this item cheapest right now?",
    "How does each realm compare?",
    "No observations yet.",
    "Which cross-realm listings are genuinely cheap?",
    "published history and latest realm snapshots",
    "{} deals after evidence gates",
    // §11 WoW Token. The compact Auction House panel receives these through
    // its view model rather than as literals in the template.
    "What is the current WoW Token price for this region?",
    "latest collection",
    "one regional token price",
    "no token price collected",
    "Not enough history yet — the chart appears after a few collections.",
    "Not enough history to show a distribution yet.",
    "Needs most of a week of observations before a weekly rhythm means anything.",
    "{} of 168 hours of the week",
    "Not enough of the ladder to draw a curve.",
    "listed right now",
    "the whole history",
    "{} paired observations",
    "one row per patch",
    "Rank {}",
    "Rank {} of {}",
    // §12 anomaly labels are values rendered beside the translated question,
    // so omitting one would create the mixed-language panel users reported.
    "Ordinary",
    "Unusual",
    "Extreme",
    // Server-rendered SVG chart tooltips never pass through Askama, so their
    // labels must be listed explicitly before chart.rs can translate them.
    "stock",
    "price",
    "median",
    "nothing collected",
    ...
    "Stock · units listed",
    "Price over time",
    "Item details are unavailable right now; prices below are unaffected.",
