//! Market tracking: price statistics, alert rules, the catalog, and one full
//! collection pass against fakes.
...
use app_core::market::{
    ...
    Copper, ItemId, Listing, NullSink, Outcome, PriceSample, Region, Snapshot, alerts, summarise,
};
...
fn catalog_lookups_resolve_ranks() {
    ...
    assert_eq!(flask.rank_of(ItemId(241324)), Some(1));
...
impl PriceRepository for FakePrices {
    ...
    async fn window_stats(
        ...
        Ok(by_item
            .into_iter()
            .map(|(item, prices)| app_core::market::WindowStats {
                item,
